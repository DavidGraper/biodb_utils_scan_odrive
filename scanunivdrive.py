...
def gettreewalkedfilenames(personid, documentdir, rootpath):
...
def scanunivdrive1(rootpath):
    ...
    for person_documentdir in person_documentdirs:
        ...
        personfilesfromdirectory = gettreewalkedfilenames(personid, persondocpath, rootpath)

        # Compare the two collections
        databaseupdates = comparefilelists(personfilesfromdb, personfilesfromdirectory)

        print("Updating {0} files for {1} {2}".format(len(databaseupdates), personfirstname, personlastname))
